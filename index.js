...
const app = async pathQuestion => {
  try {
    ...
    const writeText = ({
      ...
      write(
        getColorStr(`Вопрос №${numberQuestion + 1}. ${question}\x1b`, 'blue'),
      );
          ...
          if (gameCreationObj.questions.length > 0) {
            for (let i = 0; i < gameCreationObj.questions.length; i++) {
              ...
              if (answer === 'stop') {
                ...
                write(
                  getColorStr(
                    `Вы ответили правильно на ${gameCreationObj.correctAnswers.length > 0 ? declOfNum(Math.round((gameCreationObj.correctAnswers.length / gameCreationObj.questions.length) * 100), ['процент', 'процента', 'процентов']) : '0 процентов'} из 100%`,
                    'green',
                  ),
                );
                write(
                ...
                write(
                  getColorStr(
                    'Для выхода из игры введите команду exit',
                    'whiteBright',
                  ),
                );
                return;
